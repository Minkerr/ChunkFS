...
impl<Hash> Database<Hash> for LsmTree<Hash, Vec<u8>>
where
    Hash: ChunkHash + Ord + Serialize + for<'a> Deserialize<'a>,
{
    fn save(&mut self, segments: Vec<Segment<Hash>>) -> std::io::Result<()> {
        for segment in segments {
            self.insert(segment.hash, segment.data);
        }
        Ok(())
    }
